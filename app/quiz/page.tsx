...
const QUIZ_DATA = [
  {
    question: "Um comboio sai de Lisboa a 120 km/h. Outro sai do Porto a 80 km/h. Quando se cruzarem, qual estará mais perto de Lisboa?",
    options: ["O que saiu de Lisboa", "O que saiu do Porto", "Estarão à mesma distância", "Depende do vento"],
    correct: 2,
    hint: "Pensa bem: se eles se cruzam no mesmo ponto exato, a distância até Lisboa é igual para os dois!"
  },
  {
    question: "Se 5 máquinas levam 5 minutos a fazer 5 peças, quanto tempo levam 100 máquinas a fazer 100 peças?",
    options: ["100 minutos", "50 minutos", "5 minutos", "1 minuto"],
    correct: 2,
    hint: "A velocidade não muda. Cada máquina individual demora 5 minutos a fazer 1 peça."
  }
];
...
export default function AffectiveQuizRoute() {
  ...
  const handleAnswer = () => {
    if (selectedOpt === QUIZ_DATA[currentQ].correct) setScore(s => s + 1);
          ...
          {!quizFinished ? (
            <>
              <div className="flex justify-between items-center mb-6 md:mb-8">
                <span className="text-xs md:text-sm text-slate-500">Pergunta {currentQ + 1} de {QUIZ_DATA.length}</span>
              ...
              <h2 className="text-xl md:text-2xl font-semibold leading-relaxed mb-6 md:mb-8">{QUIZ_DATA[currentQ].question}</h2>

              {isHintActive && (
                <div className="mb-6 p-3 md:p-4 bg-sky-900/30 border border-sky-500/30 rounded-xl text-sky-200 text-sm">💡 {QUIZ_DATA[currentQ].hint}</div>
              )}

              <div className="flex flex-col gap-3 mb-auto">
                {QUIZ_DATA[currentQ].options.map((opt, i) => (
              ...
              <button onClick={handleAnswer} disabled={selectedOpt === null} className="mt-6 md:mt-8 w-full py-3 md:py-4 rounded-xl bg-slate-100 text-slate-900 font-bold hover:bg-white disabled:opacity-50 transition-all cursor-pointer text-sm md:text-base">
                Avançar
              </button>
            </>
          ) : (
            <div className="text-center my-auto py-10">
              ...
              <p className="text-slate-400">Pontuação: {score} / {QUIZ_DATA.length}</p>
            </div>
          )}
        </div>
      </div>
